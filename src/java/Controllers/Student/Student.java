...
public class Student extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ClassDAO test = (ClassDAO)session.getAttribute("classDAO");
            HashMap.Entry<Integer, Models.Classes> entry = test.getClasses().entrySet().iterator().next();
            int firstKey = entry.getKey();
            if (session.getAttribute("currentClass") == null) 
            session.setAttribute("currentClass", firstKey);
        } catch(Exception e) {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        session.setAttribute("currentClass", currentClass);
        request.setAttribute("activePage", "3");
